...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const sysPrompt = `You analyze a photo of a Panini-style football sticker album page.
Identify every sticker SLOT visible on the page. For each slot, return its sticker code
(e.g. "BRA1", "ARG10", "FWC1") and whether it is "filled" (a sticker is glued in place)
or "empty" (the slot is visible but no sticker yet).
Use ONLY codes that exist in this catalog: ${catalog.join(",")}.
If you can read a code printed on the slot but it isn't in the catalog, omit it.
Return STRICT JSON: {"slots":[{"code":"BRA1","status":"filled"|"empty"}], "page_hint": "short text"}.
Do not invent codes. Do not include duplicates.`;
