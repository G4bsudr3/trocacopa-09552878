-- Fix 1: notify_trade_status
-- O trigger anterior sempre notificava o requester_id, independente de quem disparou
-- a mudança. Quando o requester cancela, o receiver ficava sem notificação.
-- Correção: para 'accepted'/'declined' (ação do receiver) → notifica requester.
-- Para 'cancelled'/'completed' (qualquer um pode disparar) → notifica AMBOS os lados
-- (quem agiu verá a própria notificação, mas é preferível a nenhum receber).
...
BEGIN
  ...
  SELECT notification_prefs INTO prefs_req FROM public.profiles WHERE id = NEW.requester_id;
  SELECT notification_prefs INTO prefs_rec FROM public.profiles WHERE id = NEW.receiver_id;

  -- accepted / declined: ação do receiver → notifica requester
  IF NEW.status IN ('accepted', 'declined') THEN
    IF COALESCE(prefs_req->>'trades', 'true') <> 'false' THEN
      INSERT INTO public.notifications (user_id, type, payload)
      VALUES (NEW.requester_id, 'trade_' || NEW.status::text,
              jsonb_build_object('trade_id', NEW.id));
    END IF;

  -- cancelled / completed: qualquer parte pode disparar → notifica ambos
  ELSIF NEW.status IN ('cancelled', 'completed') THEN
    IF COALESCE(prefs_req->>'trades', 'true') <> 'false' THEN
      ...
      VALUES (NEW.requester_id, 'trade_' || NEW.status::text,
              jsonb_build_object('trade_id', NEW.id));
    END IF;
    IF COALESCE(prefs_rec->>'trades', 'true') <> 'false' THEN
      INSERT INTO public.notifications (user_id, type, payload)
      VALUES (NEW.receiver_id, 'trade_' || NEW.status::text,
              jsonb_build_object('trade_id', NEW.id));
    END IF;
  END IF;
...
-- Fix 2: scan_match_alerts ainda usava discoverable = true sem COALESCE.
-- O backfill já resolveu os NULLs existentes, mas novos usuários criados antes
-- do trigger de default entrar em vigor poderiam ter NULL novamente.
